  ...
  <v-card class="ma-3">
    ...
    <v-data-table
      ...
      >
        ...
        <template v-slot:[`item.std_cost`]="{ item }">
          <p v-if="item.std_cost"> Default</p>
        </template>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          small
          class="mr-2"
          @click="$router.push(`/routing-edit/${item.rtg_id}/${item.item_master_id}`)"
          v-if="authorize_edit"
          :style="{
...
approver_active_show:[],
  ...
  methods: {
    ...
    async loadRouting() {
      const result = await api.getRoutinggetAllgroupByCompanyID(localStorage.getItem(server.COMPANYID));
