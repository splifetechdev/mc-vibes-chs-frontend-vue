  ...
  <v-container fluid>
    <v-card  class="ma-3">
      ...
      <v-dialog v-model="dialogadd" max-width="500px">
        <v-card>
          <v-card-title class="text-h5">Add Division</v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
              ...
              <v-col cols="12" sm="12" md="12">
                <v-select
                  ...
                  :item-text="getnamecodedepartment"
                  item-value="id"
      ...
      <v-dialog v-model="dialogedit" max-width="500px">
        <v-card>
          <v-card-title class="text-h5">Edit Division</v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="12" sm="12" md="12">
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Division Code", value: "division_code" },
      { text: "Division Name", value: "division_name" },
 ...
 departmentShow: [],
  ...
  methods: {
    ...
    async loadDivision() {
      this.$showLoader();
      const result = await api.getAllDivisionByCompany(localStorage.getItem(server.COMPANYID));
    ...
    async savedata() {
      ...
      const res_add = await api.addDivision(this.itemadd);
    ...
    async openeditItem(item) {
      ...
      this.itemedit.code = item.division_code;
      this.itemedit.name = item.division_name;
    ...
    async savedataedit() {
      ...
      if (this.itemedit.department_id == 0) {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
      ...
      const res_update = await api.updateDivision(
        this.itemeditid,
        this.itemedit
      );
    ...
    async deleteItemConfirm() {
      const res_del = await api.DeleteDivision(this.delete_item_id);
