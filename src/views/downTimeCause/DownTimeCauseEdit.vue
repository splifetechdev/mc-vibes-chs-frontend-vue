                ...
                <div v-if="activetab === 1 && callsoopentab == true" class="tabcontent">
                  <v-row class="mb-n6">
                    <v-col cols="12" sm="12" md="4">
                      <v-text-field v-model="itemadd.reason_code" label="Reason Code *" outlined dense></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="4">
...
export default {
  ...
  async mounted() {
    ...
    await this.onLoadUdefineIDByCompanyAndModuleName(
      udgroup.DOWN_TIME_CAUSE,
      localStorage.getItem(server.COMPANYID)
    );
    this.udmodule_name = udgroup.DOWN_TIME_CAUSE;
  ...
  methods: {
    ...
    async loaddataEdit() {
      ...
      const result = await api.getDownTimeCauseByID(
        this.main_id,
      ...
      if (result.data[0]) {
        //item
        this.itemadd.reason_code = result.data[0].reason_code;
    ...
    async savedata() {
      ...
      if (this.itemadd.reason_code == null || this.itemadd.reason_code == "") {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
          "Please fill in the Reason Code",
          "text-h5 red--text text-center"
        );
        return;
      }

      if (this.itemadd.description == null || this.itemadd.description == "") {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
      ...
      const res_add = await api.updateDownTimeCause(
