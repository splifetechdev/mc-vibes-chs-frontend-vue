  ...
  <v-container fluid>
    <v-card class="ma-3">
      <v-row class="mt-5 ml-5 mr-5">
        <v-col cols="12" md="10">
          <v-toolbar flat>
            <v-toolbar-title>DownTime Cause : </v-toolbar-title>
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <v-btn
              ...
              dark
              class="mb-2 mr-2"
              @click="$router.push(`/downtime-cause-add`)"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-icon
            small
            class="mr-2"
            @click="$router.push(`/downtime-cause-edit/${item.id}`)"
            v-if="authorize_edit"
            :style="{
...
export default {
  ...
  methods: {
    ...
    async loadItemMaste() {
      const result = await api.getDownTimeCauseCompanyID(
