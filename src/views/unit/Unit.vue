  ...
  <v-container fluid>
    <v-card class="ma-3">
      ...
      <v-dialog v-model="dialogadd" max-width="500px">
        <v-card>
          <v-card-title class="text-h5">Add Unit</v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
      ...
      <v-dialog v-model="dialogedit" max-width="500px">
        <v-card>
          <v-card-title class="text-h5">Edit Unit</v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="12" sm="12" md="12">
                  <v-text-field
                    v-model="itemedit.unit_name"
                    label="Unit Name"
                    outlined
                  ></v-text-field>
                </v-col>
...
export default {
  ...
  methods: {
    ...
    async loadUnit() {
      this.$showLoader();
      const result = await api.getAllUnitByCompany(
    ...
    async savedata() {
      ...
      const res_add = await api.addUnit(this.itemadd);
    ...
    async openeditItem(item) {
      ...
      this.itemedit.unit_name = item.unit_name;
    ...
    async savedataedit() {
      ...
      const res_update = await api.updateUnit(this.itemeditid, this.itemedit);
