      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-icon
            small
            class="mr-2"
            @click="$router.push(`/holiday-edit/${item.id}`)"
            v-if="authorize_edit"
            :style="{
...
export default {
  data: () => ({
    ...
    headers: [
      {
        text: "Work Center Group",
        align: "start",
        sortable: false,
        value: "wcg_id",
      },
      { text: "Machine", value: "machine_name" },
      { text: "Date", value: "fdate_from" },
      { text: "Type", value: "holiday_type" },
  ...
  methods: {
    ...
    async loadItemMaste() {
      const result = await api.getHolidayCompanyID(
