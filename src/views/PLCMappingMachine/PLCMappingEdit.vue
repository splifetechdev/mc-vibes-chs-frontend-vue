        ...
        <v-card-title>
          <v-row>
            <v-col cols="12" md="12" justify="center" class="mb-6">
              <h3>PLC Mapping Machine Edit</h3>
            </v-col>
          </v-row>
        </v-card-title>
        <v-row>
                ...
                <div v-if="activetab === 1 && callsoopentab == true" class="tabcontent">
                  ...
                  <v-row class="mb-10 mt-6">
                    <v-col cols="12" sm="12" md="5">
                      <v-text-field v-model="itemadd.plc_id" label="PLC ID" outlined dense></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="5">
                      <v-select required outlined :items="machinelist" v-model="itemadd.machine_id" item-value="id"
                        :item-text="getgroupnamemachine" label="Machine ID" dense></v-select>
                    </v-col>
                  </v-row>
...
export default {
  ...
  async mounted() {
    ...
    await this.loadMachineList(localStorage.getItem(server.COMPANYID));
    await this.onLoadUdefineIDByCompanyAndModuleName(udgroup.PLC_MAPPING_MACHINE, localStorage.getItem(server.COMPANYID));
    this.udmodule_name = udgroup.PLC_MAPPING_MACHINE;
  ...
  methods: {
    ...
    async loaddataEdit() {
      ...
      const result = await api.getPLCMappingMachinetByID(this.main_id, this.main_u_define_module_id);
      if (result.data) {
        //item
        this.itemadd.plc_id = result.data.plc_id;
        this.itemadd.machine_id = result.data.machine_id;

        //itemdfine
        this.itemaddudefine.u_define_module_id = result.data.u_define_master.u_define_module_id;
        this.itemaddudefine.numeric1 = result.data.u_define_master.numeric1
        this.itemaddudefine.numeric2 = result.data.u_define_master.numeric2
        this.itemaddudefine.date1 = this.formatDate(result.data.u_define_master.date1);
        this.itemaddudefine.date2 = this.formatDate(result.data.u_define_master.date2);
        this.itemaddudefine.boolean1 = result.data.u_define_master.boolean1 == true ? 1 : 0;
        this.itemaddudefine.boolean2 = result.data.u_define_master.boolean2 == true ? 1 : 0;
        this.itemaddudefine.char1 = result.data.u_define_master.char1
        this.itemaddudefine.char2 = result.data.u_define_master.char2
        this.itemaddudefine.text1 = result.data.u_define_master.text1
        this.itemaddudefine.text2 = result.data.u_define_master.text2
      }
    ...
    async savedata() {
      ...
      if (this.itemadd.plc_id == "" || this.itemadd.plc_id == 0) {
      ...
      if (this.itemadd.machine_id == "" || this.itemadd.machine_id == 0) {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
      ...
      const res_add = await api.updatePLCMappingMachinet(this.$route.params.id, this.itemadd);
