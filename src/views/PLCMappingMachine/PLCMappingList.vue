<template>
  ...
  <v-card class="ma-3">
    <v-row class="mt-5 ml-5 mr-5">
          <v-col  cols="12" md="10">
            <v-toolbar flat>
            <v-toolbar-title>PLC Mapping : </v-toolbar-title>
    ...
    <v-data-table
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <v-btn
                ...
                dark
                class="mb-2 mr-2"
                @click="$router.push(`/plc-mapping-machine-add`)"
                :disabled="!authorize_add"
              >
                Add PLC Mapping Machine
              </v-btn>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          small
          class="mr-2"
          @click="$router.push(`/plc-mapping-machine-edit/${item.id}`)"
          v-if="authorize_edit"
          :style="{
...
approver_active_show:[],
  ...
  async mounted() {
    ...
    // ----------------- Check Authorize ---------------------------
    await this.loadPLCMappingMachinet();
  ...
  methods: {
    ...
    async loadPLCMappingMachinet() {
      const result = await api.getPLCMappingMachinetByCompanyID(localStorage.getItem(server.COMPANYID));
     
      this.desserts = result.data.map(machineData => ({
        ...machineData?.company,
        ...machineData?.tbl_plc_mapping_machine,
        ...machineData
      }));
      // this.desserts = result.data;
