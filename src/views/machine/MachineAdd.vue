                ...
                <div v-if="activetab === 1 && callsoopentab == true" class="tabcontent">
                  <v-row class="mb-n6">
                    <v-col cols="12" sm="12" md="5">
                      <v-text-field autofocus v-model="itemadd.machine_id" label="Machine ID" outlined
                        dense></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="5">
                ...
                <div v-if="activetab === 2" class="tabcontent">
                  ...
                  <v-data-table :headers="headers" :items="machine_shift_list" :search="search" :footer-props="{
                    ...
                    <template v-slot:top>
                      <v-toolbar flat>
                        <v-spacer></v-spacer>
                        <v-btn color="#254E58" dark class="mb-2 mr-2" @click="$router.push(`/machine-add`)"
...
export default {
  ...
  methods: {
    ...
    async addMachineShift() {
      ...
      this.machine_shift_list = [...this.machine_shift_list, { ...foundedShift, supervisor_id: foundedSupervisor.id, supervisor_name: foundedSupervisor.name }]
      this.selectedShift = null
      this.selectedSupervisor = null
    },
    async onRemoveMachineShift(machineShiftId) {
      this.machine_shift_list = this.machine_shift_list.filter(msl => msl.id !== machineShiftId)
    },
    async savedata() {
      console.log({
        ...this.itemadd,
      ...
      this.$showLoader();
      const res_add = await api.addMachine(this.itemadd);
      if (res_add.status == 200) {
        this.$hideLoader();
        this.machine_id = res_add.data.id
        ...
        if (this.machine_shift_list.length > 0) {
          await Promise.all(
            this.machine_shift_list.map(machineShift =>
              api.addMachineShift(res_add.data.id, {
                machine_id: res_add.data.id,
                shift_id: machineShift.id,
