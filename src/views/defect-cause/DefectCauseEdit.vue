                ...
                <div v-if="activetab === 1 && callsoopentab == true" class="tabcontent">
                  ...
                  <v-row class="mb-10 mt-6">
                    <v-col cols="12" sm="12" md="5">
                      <v-text-field v-model="itemadd.waste_code" label="Defect Code" outlined dense></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="5">
                      <v-text-field v-model="itemadd.description" label="Description" outlined dense></v-text-field>
                    </v-col>
                  </v-row>
...
export default {
  ...
  async mounted() {
    ...
    await this.onLoadUdefineIDByCompanyAndModuleName(udgroup.DEFECT_CAUSE, localStorage.getItem(server.COMPANYID));
    this.udmodule_name = udgroup.DEFECT_CAUSE;
  ...
  methods: {
    async loaddataEdit() {
      ...
      const result = await api.getDefectCauseByID(this.main_id, this.main_u_define_module_id);
      if (result.data[0]) {
        //item
        this.itemadd.waste_code = result.data[0].waste_code;
        this.itemadd.description = result.data[0].description;
    ...
    async savedata() {
      ...
      if (this.itemadd.waste_code == "" || this.itemadd.waste_code == 0) {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
          "Please enter Defect Code",
          "text-h5 red--text text-center"
        );
        return;
      }
      if (this.itemadd.description == "" || this.itemadd.description == 0) {
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
          "Failed!!!",
      ...
      const res_add = await api.updateDefectCause(this.$route.params.id, this.itemadd);
