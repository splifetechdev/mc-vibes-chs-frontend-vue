<template>
  ...
  <v-card class="ma-3">
    <v-row class="mt-5 ml-5 mr-5">
          <v-col  cols="12" md="10">
            <v-toolbar flat>
            <v-toolbar-title>Defect Cause : </v-toolbar-title>
    ...
    <v-data-table
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <v-btn
                ...
                dark
                class="mb-2 mr-2"
                @click="$router.push(`/defect-cause-add`)"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          small
          class="mr-2"
          @click="$router.push(`/defect-cause-edit/${item.id}`)"
          v-if="authorize_edit"
          :style="{
...
approver_active_show:[],
  ...
  methods: {
    ...
    async loadDefectCause() {
      const result = await api.getDefectCauseByCompanyID(localStorage.getItem(server.COMPANYID));
      this.desserts = result.data;
