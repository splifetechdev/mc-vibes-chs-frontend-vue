            ...
            <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
              ...
              <template v-slot:activator="{ on, attrs }">
                ...
                <v-text-field clearable @click:clear="
                  ...
                  " v-model="itemadd.entry_date" persistent-hint
                  @keydown.enter="entry_date = parseDate(itemadd.entry_date)"
                  @blur="entry_date = parseDate(itemadd.entry_date)" append-icon="mdi-calendar" label="Entry Date"
...
export default {
  ...
  methods: {
    async loaddataEdit() {
      ...
      const result = await api.getWorkerid(this.main_id);
      if (result.data) {
        ...
        this.entry_date = result.data.entry_date;
        this.itemadd.entry_date = this.formatDate(result.data.entry_date);
    ...
    async savedata() {
      ...
      this.itemadd.entry_date = this.entry_date;

      // Insert
      const res_add = await api.updateWorker(
      ...
      if (res_add.status == 200 || res_add.status == 201) {
        ...
        this.itemadd.entry_date = this.formatDate(this.itemadd.entry_date);
        this.$hideLoader();
        this.$store.state.global_dialog = true;
        this.setupAlertDialog(
          true,
